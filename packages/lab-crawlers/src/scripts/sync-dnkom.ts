...
if (!dryRun) {
  ...
  for (const promotion of promotions.promotions) {
    try {
      ...
      await logError(repository, scraperRun.id, report, 'promotion', promotion.sourceUrl, error, promotion.rawPayload);
    }
  }

  for (const item of promotions.promotionItems) {
    const promotionId = item.promotionExternalId ? promotionIdsByExternalId.get(item.promotionExternalId) : undefined;

    if (!promotionId) {
      await logError(repository, scraperRun.id, report, 'promotion_item', item.sourceUrl, new Error('promotion not found for promotion item'), item.rawPayload);
      continue;
    }
    ...
    try {
      ...
      report.promotionItemsUpserted += 1;
      await repository.logScraperRunItem({
        scraperRunId: scraperRun.id,
        providerTestId,
      ...
      await logError(repository, scraperRun.id, report, 'promotion_item', item.sourceUrl, error, item.rawPayload);
    }
  }
